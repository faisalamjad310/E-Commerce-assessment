        ...
        <div className="space-y-3">
          {orders.map(order => (
            <Link
              key={order._id}
              to={`/orders/${order._id}`}
              className="group flex items-center gap-4 bg-white dark:bg-gray-900 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm hover:shadow-md hover:border-indigo-100 dark:hover:border-indigo-900/50 p-4 transition-all"
            >
              ...
              <div className="text-right shrink-0">
                ...
                <ChevronRight className="w-4 h-4 text-gray-300 dark:text-gray-600 mt-1 ml-auto group-hover:text-indigo-400 transition-colors" />
              </div>
            </Link>
          ))}
        </div>
